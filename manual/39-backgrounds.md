...
When choosing a background with `Super + Ctrl + Space`, you can adjust the horizontal crop alignment for wide wallpapers using the `Up` and `Down` arrow keys or by clicking the on-screen alignment buttons (`Left`, `25%`, `Center`, `75%`, `Right`). Your chosen alignment is saved per image in `~/.config/omarchy/background-alignments.json` and automatically applied across both your desktop and lock screen.

You can also automate wallpaper changes with timed slideshow rotation. In the `Super + Ctrl + Space` picker, press `Tab` or click the `Slideshow` tab to select your rotation interval (`Off`, `1m`, `5m`, `15m`, `30m`, `1h`). When enabled, Omarchy automatically cycles through the current theme's wallpapers using seamless desktop transitions. You can also toggle rotation from the Omarchy Menu under _Trigger > Toggle > Background Slideshow_ or manage it from the terminal using `omarchy-theme-bg-slideshow` (`status`, `toggle`, `on 15m`, `off`, `interval 30m`). Slideshow state is saved in `~/.config/omarchy/background-slideshow.json`.
